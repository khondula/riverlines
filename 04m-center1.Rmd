...
checked_site_yr <- fs::dir_ls(glue('{data_dir}/centerlines/03_rivmosaic-imgs')) %>% 
  basename() %>% grep('LENO', x = ., value = TRUE)
checked_site_yr
...
```{r}
lines_df <- readxl::read_excel('line_segment_info.xlsx') %>%
  dplyr::filter(site_yr_id %in% checked_site_yr)
lines_df
