...
```{r}
my_riv_fill10 <- st_read(glue('{data_dir}/05_riv_smooth_polygons/{checked_site_yr}-{my_group_id}-riverfill10.shp'))
```

```{r}
# rm(my_awei)
# rm(my_awei_max)
my_mndwi_path <- glue('{data_dir}/03_watermask-rivgroups/{checked_site_yr}-{my_group_id}-MNDWI_01.tif')
...
plot(riv_mask4)
# 
# read in from Dell if needed
# my_mask_path <- glue('{data_dir}/03_watermask-rivgroups/{checked_site_yr}-{my_group_id}-MNDWI_01mask.tif')
...
leaflet() %>%
  ...
  addPolygons(data = riv4326, color = ~my_pal(rowid), group = 'riv',
   ...
   addMarkers(data = pts4326,   
             # clusterOptions = markerClusterOptions(),
             group = 'pts', popup = ~glue('point {pt_id}')) %>%
  addLayersControl(overlayGroups = c('riv', 'pts'))
```

These are all the endpoints for doing the shortest distance calculations.
Identify all the headwater and downstream points 
...
pt1 <- my_pts_sf %>% dplyr::filter(pt_id == 365)
pt2 <- my_pts_sf %>% dplyr::filter(pt_id == 371)
# plot(my_riv_fill10$geometry)
...
sf::st_crs(line_sf) <- sf::st_crs(my_pts_sf) # CRS doesn't come through?
...
```{r}
line_sf %>% st_write(glue('{checked_site_yr}-{my_group_id}-riverline7.shp'))
```
...
site_sf %>% ggplot() + geom_sf(data = my_riv_fill10, fill = 'dodgerblue', col = 'dodgerblue', lwd = 2) + geom_sf(col = 'yellow', lwd = 0.5) + theme_void()
