...
checked_site_yr <- fs::dir_ls(glue('{data_dir}/centerlines/01_watermask-imgs')) %>% basename() %>%
  grep('DELA', x = ., value = TRUE)
```
...
mask_dir <- glue('{data_dir}/centerlines/01_AWEI/{basename(my_site_dir)}')
...
m %>% writeRaster(glue('{data_dir}/centerlines/dela.tif'))
plot(m)
...
```{r}
patches2 <- terra::rast('S:/users/khondula/DATA/centerlines/09_patches/2020_YELL_3/2020_YELL_3-patches-group3b.tif')
gg3 <- ggplot() +
  # geom_sf(data = riv_smooth, col = 'red', fill = NA, lwd = 0) +
  ...
  theme_void() +
  theme(legend.position = 'none')
ggsave('yell.pdf', width = 13, height = 9)
#
