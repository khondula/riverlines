...
sort(unique(check_df$group))
```

```{r}
my_group <- sort(unique(check_df$group))[2] # NA if no groups
my_group
my_group_id <- 'clbj2'
```
...
m %>% writeRaster(glue('{data_dir}/centerlines/03_watermask-rivgroups/{my_site_yr}-{my_group_id}-MNDWI.tif'), overwrite = TRUE)
plot(m)
...
plot(m01)
m01 %>% writeRaster(glue('{data_dir}/centerlines/03_watermask-rivgroups/{my_site_yr}-{my_group_id}-MNDWI_01.tif'), overwrite = TRUE)
```
...
m <- terra::rast(glue('tmp/mosaic-{my_site_yr}.tif'))
m %>% writeRaster(glue('{data_dir}/centerlines/03_watermask-rivgroups/{my_site_yr}-{my_group_id}-AWEI.tif'), overwrite = TRUE)
plot(m)
