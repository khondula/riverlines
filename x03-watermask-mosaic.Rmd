...
```{r}
gdalcommand <- glue::glue('gdalbuildvrt mosaic.vrt {glue_collapse(group_mask_files, sep = " ")}')
system(gdalcommand)
gdalcommand2 <- 'gdal_translate -of GTiff mosaic.vrt mosaic.tif'
system(gdalcommand2)
m <- terra::rast('mosaic.tif')
plot(m)
```
...
# mosaic_outname <- glue('mosaic-{basename(my_site_dir)}-{my_group}.tif') %>% as.character()
# 
# # wbt_mosaic(output = mosaic_outname,
# #            inputs = group_mask_files,
# #            verbose_mode = TRUE,
...
wb_filename1 <- glue('{wb_dir}/rivmask_wb-{my_group}.tif')
...
fs::dir_create(wb_dir2)
wb_filename2 <- glue('{wb_dir2}/wb_dists-{my_group}.tif')
...
wb_filename1 <- glue('{my_dir8}/sobelmask20-{my_group}.tif')
riv_sobel_mask20 %>% terra::writeRaster(wb_filename1, overwrite = TRUE)
# riv_sobel_mask20 <- terra::rast(wb_filename1)
# plot(riv_sobel_mask20)

# rcl_matrix <- matrix(c(1, 1, 
                         # NA, 0), ncol = 2, byrow = TRUE)
# riv_sobel_mask20 <- terra::classify(riv_sobel_mask20, rcl = rcl_matrix)
plot(riv_sobel_mask20)

my_dir9 <- glue('09_wb_patches/{basename(my_site_dir)}')
fs::dir_create(my_dir9)

wb_filename2 <- glue('{my_dir9}/patches-{my_group}.tif')
...
riv_sobel_mask20_patches %>% terra::writeRaster(glue('{wb_dir9c}/patches-{my_group}.tif'))
riv_sobel_mask20_patches <- terra::rast(glue('{wb_dir9c}/patches-{my_group}.tif'))
# plot(riv_sobel_mask20_patches)
gg3 <- ggplot() +
  ...
  theme_void()
#
ggsave(glue('{wb_dir9c}/{basename(my_site_dir)}-patches-{my_group}.pdf'), gg3, width = 6, height = 12)
