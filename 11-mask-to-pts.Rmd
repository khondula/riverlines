...
save_watermask <- function(my_site_file){
  ...
  my_wm_dir <- glue('{data_dir}/centerlines/02_watermask/{my_site_yr}')
...
check_df %>% readr::write_csv(glue('{data_dir}/riverline-checks/{my_site_yr}.csv'))
```
...
```{r}
rlist <- river_site_files[1:21] %>%
  purrr::map(~readRDS(.x)) %>%
  purrr::map(~terra::rast(.x))
rsrc <- terra::src(rlist)
m <- terra::mosaic(rsrc)
wrap(m) %>% saveRDS('blan-mosaic.rds')
plot(m)
...
mm <- ggplot() +
    ...
    theme_void() +
    theme(legend.position = 'bottom')
ggsave(filename = 'blan2021-mosaic.png', mm)
ggsave(filename = 'blan2021-mosaic.pdf', mm, width = 20, height = 30)
```
...
```{r}
  my_mndwi <- readRDS(file = 'blan-mosaic.rds') %>% terra::rast()
...
```{r}
riv_smooth <- st_read('polygons/BLAN_2021/blan_mosaic-riversmooth.shp')
riv_smooth_sv <- as(riv_smooth, "SpatVector")
ggplot() +
